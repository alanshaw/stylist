Style the element with the ID "box" so that the total space occupied by the element is `200px` by `100px`. You **must** set values for CSS properties `width`, `height`, `padding`, `border` and `margin`. Lastly, ensure the border is dashed and red.
...
See documentation on the box model for more info:
